window.Sorter = (function () {
  ...
  var generateArray = function () {
    ...
    //visualizableArray = [200,900,200,100,300,700,500,400,100,800];
  ...
  var quickSort = function(quickArray,startIndex) {
    ...
    if(left.length > 1 ) {
      ...
      //setTimeout(quickTransformDisplayItems,interval,left,startIndex);
    }

    if(right.length > 1 ) {

      right = quickSort(right, startIndex+left.length);
      //setTimeout(quickTransformDisplayItems,interval,right,startIndex+left.length);
    }

    var out = left.concat(pivotValue,right);

    quickTransformDisplayItems(out,startIndex);
    //setTimeout(quickTransformDisplayItems,interval,out,startIndex);
